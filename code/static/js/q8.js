...
const crisisColors = d3.schemePastel1;
...
function update(data) {
  ...
  const crisis = data.crisis_periods;
  crisis.forEach((d, i) => {
    svg.append("rect")
      .attr("x", x(d.start))
      .attr("y", 0)
      ...
      .attr("height", height)
      .attr("fill", crisisColors[i % crisisColors.length])
      .attr("opacity", 0.5);
  });
  ...
  if (!showSubcategories) {
    ...
    legend.append("text")
      .attr("y", 0)
      .text("times of famine or unemployment")
    ...
    crisis.forEach((d, i) => {
      ...
      row.append("text")
        ...
        .attr("y", 10)
        .style("font-size", "13px")
        .text(`${d.label} (${d.start}–${d.end})`);
    });

    legend.append("text")
      .attr("y", (crisis.length + 2) * 20)
    ...
    domainKeys.forEach((key, i) => {
      const row = legend.append("g").attr("transform", `translate(0, ${(crisis.length + 1 + i) * 20})`);
      row.append("rect")
        .attr("width", 12)
        .attr("height", 12)
        .attr("fill", color(key));
...
d3.json("/static/data/q8_necessity_crime_trends.json").then(data => {
  function render() {
    selectedCategory = document.getElementById("q8Selector").value;
    showSubcategories = document.getElementById("subcategoryToggle").checked;
    update(data);
  }

  document.getElementById("q8Selector").addEventListener("change", render);
